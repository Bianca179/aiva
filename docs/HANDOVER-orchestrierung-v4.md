...
- **⚠ FEHLGRIFF + ROLLBACK (gleicher Tag):** `+15715865442` wurde als Sophias Nummer nach ElevenLabs importiert und der Trigger aktiviert — **falsch: die Nummer gehört zu einem KUNDEN-Demo-Projekt** (Biancas Stopp). Import gelöscht (DELETE 204, Kontroll-GET: Liste leer), Trigger **unpublished**, `PLATZHALTER_PHONE_NUMBER_ID` wiederhergestellt. **Lehre: Nummern im Twilio-Konto nie nach „frei/belegt" zuordnen — nur nach Biancas expliziter Ansage.**
