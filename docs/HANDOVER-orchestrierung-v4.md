...
## 24 · Befund 22.09.2026 — Marketing-Team „produktionsfähig automatisiert" (Bestandsaufnahme, NICHTS gebaut)

Anlass: Thema „Marketing-Team produktionsfähig machen, automatisiert". Nach Regel 1 zuerst Live-Stand in n8n + Airtable geprüft (read-only, zwei Workflow-Tiefenanalysen: Linni-Kette + Cockpit). **Kein GO vorhanden → nichts verändert, keine Datensätze angefasst.** Dieser Abschnitt ist die Entscheidungsgrundlage.

### 24.1 Was heute läuft (alle Schedules feuern täglich, alle `success`)
| Schritt | Workflow | Trigger | Status 22.09. |
|---|---|---|---|
| Auftrag → Ideen | `ORCH - Marketing-Eingang (Max/CC Top) - v1` `0mJg6hf46o29NdsD` | alle 15 Min (#max) | aktiv, 2.882 Läufe, liest #max korrekt |
| Recherche → Ideen | `ORCH - Marketing Recherche - v1` `dXHZnY0KaS9iNgSo` | Mo 08:00 | **INAKTIV** (SerpAPI-Tool ohne Credential, kein `serpApi`-Cred im Account) |
| Themenwahl | — | — | **existiert nirgends** (siehe 24.2 #1) |
| Produktion (CC Top + Ina/Linni/Soreia/Podcast/Vera + Nora/Claudia/Selma) | `ORCH - Marketing Produktion - v1` `9pi1p59HQWc6ASXJ` | tgl. 09:00, Filter `{Status}="Ausgewählt"` | aktiv, **findet seit 12.08. jeden Tag 0 Einträge** (Exec 15525: „Ideen suchen" → leer) |
| Kanal-Agentinnen | Ina `ZToSb9K4kbu3olf6` · Soreia `UBv3GFZnIBZ5Sc7V` · Podcast `3a8YdcWUtj2cMRbc` | Sub-Workflow (von CC Top) | aktiv, **0 Executions je** |
| Freigabe (Bianca) | Cockpit `HPl4FtmXeISou9FN` (Marketing-Karte) | Klick | aktiv, Buttons nur „Freigeben/Verwerfen" |
| Freigegeben → Slot | Linni `11UxePeldz86cqxp` (Schedule-Kette, 4 Nodes) | tgl. 08:34 | aktiv, **findet seit Wochen 0** (Exec 15515) |
| Slot → Bild | `ORCH - LinkedIn Bild erzeugen - v1` `sxqFjkGedC1ph17C` | tgl. 08:40 | aktiv, findet 0 Slots |
| Slot → LinkedIn | `ORCH - LinkedIn Veröffentlichen - v1` `NINDaWVJOWJuCvrA` | tgl. 08:45 | aktiv, findet 0 Slots |

**Zahlen Redaktionsplan `tbld1fEJeD29wy4PT` (15 Einträge):** Idee 4 · Entwurf 2 (02.08., Podcast + Instagram) · **Freigegeben 7** · Verworfen 1 (Testpost) · **Ausgewählt 0 · Wartet auf Freigabe 0 · Wartet auf Themenwahl 0**. LinkedIn Posts `tbljprv2gQO8WoGuB`: 2 Slots, beide „Gepostet" (02.08. Test, 11.08. „Arbeit, die dich nicht mehr an jeder Stelle braucht" — der **einzige echte Post des Systems**, Slot damals von Hand angelegt). Instagram Posts: 0. **Seit 11.08. hat das Marketing-Team nichts mehr veröffentlicht.**

### 24.2 Warum nichts rauskommt — 6 konkrete Brüche in der Kette
1. **Themenwahl ist nicht bedienbar (Hauptbruch).** Die Produktion wartet auf `Ausgewählt`, aber kein Workflow und kein Button setzt diesen Status. Das Cockpit zeigt `Idee` und `Wartet auf Freigabe` gemischt in EINER Liste und bietet nur „✅ Freigeben" (→ `Freigegeben`) / „Verwerfen". Bianca hat am 10./11.08. Ideen „freigegeben" — die übersprangen damit CC Top, Kanal-Agentin und Personas komplett.
2. **6 der 7 „Freigegebenen" haben KEINEN Text** (`Text/Entwurf` leer, nur Titel + CC-Top-Empfehlung): recAXQ8jAyPJjt2WO, recidKs3kRsRHZVEZ, recobLCupsURKa1y1, recrbyIHiDfKK62Bo (LinkedIn) · recUkS2M3hmVECoT1, recWUURC2oWJwZWQC (Substack). Nur recam1Tu5pogR9Zqj (Newsletter, 02.08.) hat Text. Würde die Linni-Kette sie je abholen, ginge ein **leerer Post** an Unipile (Veröffentlichen sendet `Text/Entwurf` ungeprüft).
3. **Linni-Übergabe verlangt ein Datum, das niemand setzt.** Filter wörtlich: `AND({Status}="Freigegeben", {Kanal}="LinkedIn", IS_SAME({Datum geplant}, TODAY(), 'day'))`. Weder Eingang, Produktion noch Cockpit schreiben `Datum geplant`. Freigaben ohne Datum bleiben ewig liegen; ein Datum in der Vergangenheit (spät freigegeben) matcht nie mehr. Zusätzlich setzt die Kette `Datum gepostet` schon beim Slot-Anlegen (nicht beim Posten), und **Veröffentlichen schreibt nur den Slot auf „Gepostet", nie den Redaktionsplan** (bleibt „Geplant", kein Link zum Post).
4. **Cockpit zeigt bei „Wartet auf Freigabe" den Entwurfstext NICHT** — nur `CC Top Empfehlung` aufklappbar. Bianca würde Texte freigeben, die sie nie gelesen hat. Außerdem: harte Kappung auf 12 Einträge ohne Hinweis, Status `Wartet auf Themenwahl` (Ergebnis der Recherche-Vorauswahl) ist im Cockpit unsichtbar.
5. **Guardrail-Loch im Linni-Chat-Teil („Manni"):** Das Airtable-Tool „Create a record in Airtable" (Redaktionsplan) hat `Status`-Default **„Freigegeben"** + Kanal LinkedIn + `Datum geplant` per `$fromAI`. Ein Manni-Aufruf (Bianca per Slack-DM ODER CC Top über das Produktion-Tool „Linni (LinkedIn)"!) kann damit einen Eintrag anlegen, der am nächsten Morgen 08:34 ohne Biancas Freigabe zum Slot und 08:45 auf LinkedIn wird. Das ist der einzige verbliebene Autopost-Pfad am Gate vorbei (§11 hatte nur den Such-Filter gehärtet, nicht den Tool-Default).
6. **Keine Recherche-Zufuhr:** `dXHZnY0KaS9iNgSo` (Peter Parker/Findus + Trend-Scout + CC-Top-Vorauswahl) ist fertig gebaut, scheitert nur am fehlenden SerpAPI-Key. **Alternative ohne eigenen Key:** n8n-Gateway-Credits decken **Brave Search** (`@brave/n8n-nodes-brave-search.braveSearch`) ab — Tool-Tausch statt Key-Beschaffung.

**Nebenbefunde (nicht blockierend):** Instagram/Substack/Podcast/Video haben keine Veröffentlichungs-Strecke (bekannt, §14.6) — Freigaben dort sind heute „Copy & Paste durch Bianca". Bild-Prompt nur aus Titel (bekannt). Produktion nutzt `claude-sonnet-4-6` (ok). Cockpit-Key steht im Klartext im Code (wie gebaut, unverändert).

### 24.3 Vorschlag „Produktionsfähig" — minimaler Umbau, 4 Blöcke (je ein GO)
**A — Cockpit-Marketing-Karte in zwei Stufen (`HPl4FtmXeISou9FN`, nur HTML-Bauer + „Aktion pruefen"):**
- Liste 1 „Themenwahl": Status `Idee` + `Wartet auf Themenwahl` → Buttons **„🎯 Produzieren"** (`aktion=produzieren` → `Ausgewählt`) und „Verwerfen". **Kein Freigeben auf Ideen mehr.**
- Liste 2 „Zur Freigabe": nur `Wartet auf Freigabe` → **Volltext `Text/Entwurf` + Nora-Feedback + Runden aufklappbar** → „✅ Freigeben" / „Verwerfen" / optional „↩ Überarbeiten" (→ `Ausgewählt`, läuft nochmal durch CC Top).
- Server-Guard in „Aktion pruefen": `freigeben` für `plan` nur, wenn der Datensatz `Text/Entwurf` hat (ein Airtable-Get davor, ~1 Node).
**B — Linni-Übergabe + Veröffentlichen robust machen (`11UxePeldz86cqxp` Schedule-Kette, `NINDaWVJOWJuCvrA`):**
- Filter: `AND({Status}="Freigegeben", {Kanal}="LinkedIn", LEN({Text/Entwurf}&"")>0, OR({Datum geplant}=BLANK(), IS_BEFORE({Datum geplant}, TOMORROW())))`, **Limit 1/Lauf** (= max 1 LinkedIn-Post pro Tag; Wochen-Batch verteilt sich automatisch, wie bei Sam).
- Slot-Datum = `Datum geplant` oder heute; `Datum gepostet` NICHT mehr beim Slot setzen.
- Veröffentlichen zusätzlich: Redaktionsplan `Status=Gepostet`, `Datum gepostet=heute`, `Link zum Post`. Fehler → bleibt „Geplant" + Slack-Alert (existiert schon).
**C — Guardrail Manni-Tool:** Status-Default in „Create a record in Airtable" + „Update record in Airtable" von „Freigegeben" auf **„Entwurf"** (ein Parameter je Node). Damit kann kein Agent mehr „Freigegeben" erzeugen — das Wort bleibt Biancas.
**D — Recherche aktivieren:** SerpAPI-Tools durch Brave-Search (Gateway) ersetzen, Montag 08:00 aktivieren; Vorauswahl-Ergebnis erscheint über A in „Themenwahl". Optional: Trend-/News-Rohtexte als `Kanal=Sonstiges` bleiben nur Ideen-Futter, werden nie selbst gepostet (Filter in B schützt).

**E — Datenbereinigung (Biancas Entscheidung, kein Bau):** die 6 leeren „Freigegebenen" auf `Ausgewählt` zurücksetzen (→ Produktion erzeugt am nächsten 09:00 echte Entwürfe, dann echte Freigabe) ODER verwerfen. Empfehlung: die 4 LinkedIn auf `Ausgewählt`, die 2 Substack ebenfalls (Soreia schreibt, Veröffentlichung bleibt manuell). `recam1Tu5pogR9Zqj` (Newsletter mit Text, 02.08.) und die 2 „Entwurf" von 02.08. sind Altlast aus dem 02.08.-Test — Vorschlag: verwerfen oder ebenfalls `Ausgewählt`.

**Zielbild nach A–D (vollautomatisch, ein manueller Schritt wie bei Sam):** Mo 08:00 Recherche → Ideen + CC-Top-Vorauswahl · #max jederzeit → Ideen · **Bianca: Themenwahl-Klick** (Cockpit) · 09:00 Produktion → „Wartet auf Freigabe" · **Bianca: Freigabe-Klick nach Lesen des Textes** · 08:34 Slot (1/Tag) → 08:40 Bild → 08:45 LinkedIn → Redaktionsplan „Gepostet" + Link. Wochenreport-Zeile fürs Marketing (Gepostet/Wartet/Ideen) kann in `ih2dTBAkGkUPLnOY` nachgezogen werden (später, eigenes GO).
