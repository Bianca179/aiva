...
- **Cartoon-Avatar (offen):** Ordner mit KI-Porträts liegt vor (`1J44wxBRf-c_MchojpXlnkTSSTQ4gInsJ`, Replicate-Generationen + Unterordner „Bilder für magnigifc"; gutes Basis-Porträt gesichtet: `out-1 (24).png`). Cartoonisierung ist mit den Session-Tools nicht verlässlich machbar → Bianca generiert mit ihrem Replicate-Modell oder Magnific eine Comic-Version (Prompt-Vorschlag im Chat geliefert), legt sie in den Ordner → dann Widget-Avatar setzen (platform_settings.widget.avatar / custom_avatar_path).
