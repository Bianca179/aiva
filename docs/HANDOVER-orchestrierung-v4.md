...
- **Offen (Go ausstehend): Bauplan „Marketing-Eingang v1"** — Max existiert nur als Stellenbeschreibung + leerer Kanal, KEIN Workflow liest #max. Vorschlag an Bianca: n8n-Workflow mit Slack-Trigger auf #max (Max-Logik CMO → CC-Top-Logik → Redaktionsplan-Einträge → Antwort in #max), Stimme = vorhandene Donna-App (keine eigene Max-App, gemäß Architektur-Entscheidung 28.07.: eigener Bot nur bei echten Beziehungen); im selben Zug Linnis ungetestete 140-Node-Strecke mit EINEM Post E2E testen.
