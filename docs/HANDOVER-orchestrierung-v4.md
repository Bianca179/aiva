...
### 23.5 UMGESETZT 22.09. (Biancas GOs zu a–e): Angebot A = Entwurf + Freigabe im Cockpit, Chat-Leandra kann A auslösen
**Biancas Antworten:** (a) Formular-Test erledigt → Exec 15572 = Lead „Annetta Reiner" (Spur C, Rückruf vorbereitet, Testdaten von ihr) — **Kanal lebt.** (b) Future-Self anhaken = ja; Voca-Preis **79 €**. (c) Freigabe: im Dashboard als Entwurf mit Freigabe-Knopf. (d) Robustheits-Fixes: GO. (e) Chat-Leandra soll Angebot A auslösen: ja.

**Airtable (Produkte):** Future-Self `rec6HeCrzGtGqr1OS` Haken „Einstiegsangebot (A-Zweig)" gesetzt; Voca `recKc9MeUtzKEp5zD` Preis 79 (Notiz ergänzt). → A-Angebote jetzt: Identitätscheck 49 €, Voca 79 €, Future-Self 149 €. Neue Lead-Status-Option **„Angebot gesendet (freigegeben)"** (`selXGeGcFG4RSpTab`, per typecast entstanden).

**`FmC7exobAoPLIdVK` (publiziert `916bdf48`) — A-Zweig kein Direktversand mehr:** Route(A) → Angebote laden → Kontext → „KI: Angebot erstellen (A)" (Prompt: Entwurf-Modus, interne Notiz-Hinweise nicht in die Mail, keine „automatisch erstellt"-Zeile) → **NEU „A: Text pruefen"** (Code: `merged_response || content-Text`, Code-Fences weg, Guard: <80 Zeichen oder kein HTML → `ok:false` + ⚠-Hinweistext statt leerem Entwurf) → „A: Status + Angebot speichern" (**Lead-Status „Entwurf – Wartet auf Freigabe"** + Angebot-Entwurf = HTML) → **NEU „A: Info an Bianca"** (Slack-DM via Donna `K59QwGFIa6Oqk2gm` an `U094G4R4W2X`, mit Cockpit-Link; bei ok:false ⚠-Variante). **Gmail-Node „Angebot an Lead (A)" ENTFERNT.** Sticky Note + Beschreibung aktualisiert. Belegt: Exec 15587 (Testlead „TEST A-Zweig Freigabe", Anliegen Einstieg) → Spur A, Entwurf mit Voca 79 € + FOUNDER30 + Identitätscheck 49 € + Zeeg, Status Entwurf, Slack ok:true.

**NEU `ORCH - Leandra A-Versand (nach Freigabe) - v1` (`EijvF4q0Eomfct7Y`, AKTIV, publiziert `b260a0a1`):** `GET /webhook/angebot-a-freigabe?key=7f7ac47671ef94fa&lead=recXXX&aktion=senden|verwerfen` → Zugangs-Check VOR jedem Schreibvorgang → Lead holen → „Pruefen" (nur Status „Entwurf – Wartet auf Freigabe", Spur A, gültige E-Mail, Entwurf ≥80 Zeichen + HTML; Betreff „Dein Einstieg bei Bianca Enderlin, {Vorname}") → Switch: **senden** = Gmail HTML an Lead (Absender „Bianca Enderlin") → Status „Angebot gesendet (freigegeben)" → Redirect Cockpit · **verwerfen** = Status „Erledigt" → Redirect · **fehler** = Klartext-Antwort (nichts gesendet). Belegt: Exec 15590 (senden → Mail-ID `1a0c8aa57ab76953` an bianca@, Status gesetzt), Exec 15596 (verwerfen → Erledigt). Doppelklick-Schutz: zweiter Klick findet Status ≠ Entwurf → Fehlertext, keine zweite Mail.

**Cockpit `HPl4FtmXeISou9FN` (publiziert `753af27b`, Stand `d022c038` gelesen + daraufgesetzt, keine Kollision):** neue KPI „A-Angebote zur Freigabe"; in der Empfangs-Karte neuer Block **„A-Leads · Angebot freigeben"**: Name/Firma/Anliegen/E-Mail, Entwurf aufklappbar als **gerenderte Mail** (Tag-Whitelist h3/p/ul/ol/li/strong/em/br/a, on*-Attribute + javascript: gestrippt), Knöpfe **„✅ Freigeben & senden"** (nur wenn Entwurf vorhanden) und „Verwerfen". Live verifiziert (Exec 15594: Testlead sichtbar mit beiden Knöpfen; nach Verwerfen KPI 0).

**Chat-Leandra (`agent_2601kzrnj6w1fh4b4fr0srrfcg70`) — neues Werkzeug (per API über TMP-WF, danach archiviert):** Workspace-Tool **`einstiegsangebot_anfordern` `tool_3801m34avd6eeh8a9qnq4q3smhp2`** = POST `/webhook/website-lead` mit `name`, `email`, `nachricht` (Pflicht), `organisation`, `telefon` (optional) und **Konstanten** `anliegen="Identitätscheck oder KI-Coach (Einstieg)"`, `leadClass="A"`, `secret` (liegt serverseitig im Tool, nicht im Chat). Läuft damit durch die normale Qualifizierung (→ A, außer Firmengröße ≥20/Budget-Override) → Entwurf → Freigabe im Cockpit. Agent per PATCH (Deep-Merge) erweitert: tool_ids jetzt 6, Prompt 2943→3918 Zeichen: neuer Block „[Kleiner Einstieg – in jedem Weg möglich]" (Identitätscheck 49 / Voca 79 / Future-Self 149, max. zwei nennen, Einverständnis + Name + E-Mail, „Bianca prüft und schickt", kein Sofortversand versprechen) + Werkzeug-Zeile. Per GET verifiziert (Exec 15598). **ElevenLabs-Lehre:** bei Body-Properties gilt „nur EINES von description / constant_value / dynamic_variable" — Konstanten ohne description anlegen (erster Versuch 15592 scheiterte genau daran).

**Aufgeräumt:** Testlead gelöscht; TMP-WFs `Pg1ehJeqqSICO9GI` + `8EmDJ4kUNOhyuw6c` archiviert (kein Webhook, nichts offen). **In Biancas Postfach liegt eine Test-Mail** „Dein Einstieg bei Bianca Enderlin, TEST" (vom Sende-Test) — kann weg.

**Offen / Hinweise:** (1) Leandras traffic-lose Zweitstrecke `sGGeiWqO1BJMxhJe` bleibt unangetastet. (2) Alte Status-Option „Angebot gesendet (automatisch)" bleibt für Historie, wird nicht mehr geschrieben. (3) Chat-Tool noch nicht im echten Chat durchgespielt — Bianca: im Website-Chat „Ich will klein anfangen" → Leandra sollte Einstiegsangebote nennen und nach Name/E-Mail fragen → Entwurf erscheint im Cockpit + Slack-DM.

### 23.6 B-Zweig-Bug durch Biancas Test gefunden + gefixt (22.09., „go alle")
**Fund:** Biancas Formular-Test (Exec 15600, Anliegen „Etwas anderes" → korrekt Spur B, kein A-Test) → „Sophia ruft an (B)" bekam **404 `document_not_found` für `phnum_0101kznw8e52et6twgq515pjr74e`** — die alte US-Nummer, die seit dem Umzug auf die DE-Festnetznummer (20.15) nicht mehr existiert. **Folge seit 12.08.: JEDER B-Website-Lead scheiterte beim Sophia-Anruf** (nur die ehrliche „FEHLGESCHLAGEN"-Mail ging raus), Status stand trotzdem falsch auf „Anruf gestartet". Verifiziert per ElevenLabs `GET /v1/convai/phone-numbers` (TMP-WF `ttOuyoQXVfvGAdUA`, archiviert): nur noch **`+4971564229016` „AIVas Team" `phnum_4901kzvfk5vkf80swp390xmz0rq3`**, inbound+outbound, Twilio, Donna zugewiesen. **Sophias eigener Trigger `i9JHfn8I4jmKkmPR` nutzte bereits die richtige ID** (kein Fix nötig — der Verdacht aus §22 ist damit geklärt).

**Fix `FmC7exobAoPLIdVK` (publiziert `69373688`):**
1. „Sophia ruft an (B)": `agent_phone_number_id` → `phnum_4901…` (DE-Nummer).
2. „Lead speichern" setzt B jetzt auf **„Anruf offen"** (statt „Anruf gestartet" vor dem Versuch). Neuer Node **„B: Status nach Anruf"** hinter dem Sophia-Call: `success` → „Anruf gestartet", sonst bleibt „Anruf offen" + **Anruf-Notiz** mit Fehlergrund und Zeitstempel. Nicht gestartete Anrufe holt der Sophia-Trigger `i9JHfn8I4jmKkmPR` werktags 10/14/17 Uhr nach (Filter: Status „Anruf offen" UND Telefon vorhanden).
3. „Mail: Jetzt anrufen (B)" liest Erfolg/Fehler explizit aus `$('Sophia ruft an (B)')` (Regel: eingefügter Node ändert `$json`) und zeigt `error.message` statt `[object Object]`; Text erklärt den Retry. Sticky Note aktualisiert.
**Belegt (Exec 15604, Testlead ohne Nummer → ElevenLabs 400 „No 'To' number", niemand angerufen):** Status „Anruf offen", Anruf-Notiz „Automatischer Anruf fehlgeschlagen (HTTP 400 …) – 22.09.2026 13:04", Mail gesendet. Testlead gelöscht. Erfolgsfall (echter Anruf) bewusst nicht simuliert — nächster echter B-Lead mit Nummer ist der Live-Test.

**Offen:** Biancas Test-Lead „marietta" (`recVYDm7dbXyxs7rh`, B, ohne Telefon, Status noch „Anruf gestartet" vom alten Pfad) — löschen oder auf „Erledigt" setzen, Biancas Entscheidung. In ihrem Postfach liegen zwei B-Test-Mails („ANRUF FEHLGESCHLAGEN"). Der A-Weg wurde per Formular noch nicht von Bianca selbst ausgelöst (Anliegen „Identitätscheck oder KI-Coach (Einstieg)" wählen oder Website-Chat).
